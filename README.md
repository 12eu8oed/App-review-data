# 앱사용자 리뷰 데이터 추출기 (Play Store Review Exporter) 🚀
...
- **Vite PWA**: Progressive Web App(PWA) 지원을 통해 데스크탑 및 모바일 기기에 앱처럼 설치 가능.
...
2. **정교한 필터링 & 검색**: 평점(1~5점), 기간, **특정 키워드(단어)** 별 실시간 필터링 기능.
3. **엑셀 내보내기**: 필터링된 결과물만 선택적으로 엑셀 파일로 저장하여 업무 활용도 극대화.
4. **리뷰 상세 보기**: 목록에서 행 클릭 시 전체 리뷰 내용 및 개발자 답변을 팝업으로 확인.
5. **사용자 편의 기능**: 커스텀 드롭다운을 통한 직관적인 수집 건수 설정 및 최근 검색 기록 제공.
6. **앱 설치 지원 (PWA)**: 브라우저 환경에서 네이티브 앱처럼 PC 및 스마트폰 홈 화면에 설치 가능 (커스텀 아이콘 적용).
7. **반응형 디자인**: 모바일, 태블릿, 데스크탑 등 모든 기기 환경에 최적화된 레이아웃.
